                            ...
                            <table class="tableku align-items-center justify-content-center my-0 tablesorter table-hover striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Merek</th>
                                        <th>Model Type</th>
                                        <th>Penagung Jawab Asset</th>
                                        <th>Image</th>
                                        <th>Default Agent</th>
                                        <th>Default Supervisor</th>
                                ...
                                <tbody>
                                    @foreach($assets as $asset)
                                        <tr>
                                            <td>{{ $asset->tag_asset }}</td>
                                            <td>{{ $asset->status }}</td>
                                            <td>{{ $asset->nama_item }}</td>
                                            <td>{{ $asset->condition_asset }}</td>
                                            <td>{{ $asset->deskripsi }}</td>
                                            <td>{{ $asset->merek }}</td>
                                            <td>{{ $asset->model_type }}</td>
                                            <td>{{ $asset->asset_custodian }}</td>
                                            <td>

                                                @if (!empty($asset->image))
                                                    <a class="btn btn-sm btn-outline-info m-0 p-1" style="width: 5rem"
                                                        href="{{ url('storage/' .$asset->image) }}"
                                                        target="_blank">
                                                        <img class="img-thumbnail"
                                                            src="{{ url('storage/' . $asset->image) }}"
                                                            alt="">
                                                    </a>
                                                @endif
                                            </td>
                                            <td>{{ $asset->agent->name }}</td>
                                            <td>{{ $asset->supervisor->name }}</td>
                                            <td class="text-center">
                                                    <a href="{{ route('asset-item.edit', $asset->id) }}" class="btn btn-sm btn-outline-warning">Edit</a>
                                                    {{-- <form action="{{ route('asset-item.destroy', $asset->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                ...
                buttons: [
                    {
                        ...
                        action: function ( e, dt, node, config ) {
                            // Define the URL for adding a new item
                            var url = @json(route('asset-item.create'));
